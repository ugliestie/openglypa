...
async def get_automatic_settings(group_id):
    row = cur.execute('SELECT text, topor, demotivators, memes, polls, reactions, stickers FROM group_automatic_settings WHERE group_id = ?', (group_id,)).fetchone()
    if row is None:
        await check_group(group_id)
...
async def set_setting(type_setting, kind_setting, setting, group_id):
    if type_setting == "commands":
        ...
        elif kind_setting == "reactions":
            cur.execute('UPDATE group_automatic_settings SET reactions = ? WHERE group_id = ?', (setting, group_id))
        elif kind_setting == "stickers":
            cur.execute('UPDATE group_automatic_settings SET stickers = ? WHERE group_id = ?', (setting, group_id))
    db.commit()

async def get_automatic_generations(group_id):
    row = cur.execute('SELECT lazyness, text, topor, demotivators, memes, polls, reactions, stickers FROM group_automatic_settings WHERE group_id = ?', (group_id,)).fetchone()
    if row is None:
        await check_group(group_id)
        return (95, 1, 1, 1, 1, 1, 1, 1)
    return row
